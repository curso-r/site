...
author: ["Athos"]
summary: "WoE (weight of evidence) é uma ferramenta bastante usada quando em aplicações de regressão logística, principalmente na área de score de crédito. O pacote tidywoe oferece uma interface simples para facilitar essa tarefa."
---

WoE (weight of evidence) é uma ferramenta bastante usada quando em aplicações de regressão logística, principalmente na área de score de crédito. Simploriamente falando, ele transforma categorias em números que refletem a diferença entre elas pelo critério de separação do **Y = 1** e **Y = 0**.
...
O autor desse texto é o Kim Larsen, criador do pacote [Information](https://cran.r-project.org/web/packages/Information/index.html) que é completo e cheio de ferramentas sofisticadas em torno do WoE.

No dia a dia do meu trabalho, volta e meia eu tinha que construir versões em WoE das minhas variáveis e desenvolvi umas rotinas para me ajudar nisso, mesmo com vários pacotes completos disponíveis. A principal motivação era que eles não eram muito práticos. Ainda mais depois que veio a onda do tidyverse e fuquei mimado.

Daí acabei empacotando essas rotinas num pacote chamado `tidywoe` e deixando no ar pra quem tiver interesse.

## Instalação

Rode abaixo para instalar.
...
Pretendo colocar bastante coisa nova no pacote ainda, mas já está funcionando e me ajudando no trabalho. Aqui está o [github do pacote](https://github.com/Athospd/tidywoe), contribuições são bem vindas!

...
Claro que sim! Modelos estatísticos sempre podem ter propriedades preditivas. Um modelo preditivo para as eleições, no entanto, deve utilizar muito mais informações do que apenas as intenções de voto dos eleitores. Alguns exemplos
de dados adicionais que poderiam ser utilizados:

- Historicamente, sabemos que indecisos têm uma propensão maior a votar em candidatos do tipo $A$ do que $B$. Podemos utilizar essa informação para ponderar os não respondentes.
- A proporção de brancos e nulos tradicionalmente observada nas urnas é diferente do que a das que aparecem nas pesquisas. Podemos utilizar esse dado para predizer a proporção de votos brancos e nulos. 
...
- Podemos utilizar os dados de diversas pesquisas eleitorais ao mesmo tempo, dando pesos diferentes para elas. 
...
O problema é que a avaliação das pesquisas sempre acaba sendo subjetiva. Por exemplo, sempre é possível argumentar que uma pesquisa é ruim por não ter utilizado amostragem aleatória simples (AAS) para realização do estudo, o que gera um debate incessante. Assim, me parece que a forma mais adequada de avaliar pesquisas de intenção de voto é o atendimento a requisitos formais, como o clarificação sobre o método de condução do estudo. Isso pode ser atigindo com a 
disponibilização de:

- Base de dados utilizada para realização da pesquisa e `script` (caso exista) para reprodução dos resultados.
...
- Não devemos tentar prever os próximos governantes do país a partir de resultados de pesquisas eleitorais: este não é atual objetivo delas.
...
- Estudos preditivos de resultados de eleições é o que realmente respondem ao que a população quer saber.

...
Hoje mais uma vez fui salvo por uma feature pensada no universo `tidy`. Dessa vez, o grande culpado foi o pacote [`sf`](https://github.com/r-spatial/sf), um pacote ainda em estágio de desenvolvimento mas que já considero pacas.

O `sf`, aka Simple Features, é um pacote para trabalhar com mapas. Com ele é possível fazer projeções, gráficos, leitura/gravação de diversos formatos etc. Esse é um dos pacotes patrocinados pelo [R Consortium](https://www.r-consortium.org/), uma iniciativa de várias empresas para injetar dinheiro em projetos do R com alta relevância.
...
Eu faço parte da Associação Brasileira de Jurimetria (ABJ), uma entidade que faz pesquisa aplicada na área do Direito. Na ABJ produzimos diversas soluções tecnológicas para resolver problemas do Direito, principalmente no que diz respeito à elaboração de políticas públicas e administração dos tribunais.

Num projeto recente, eu precisava levantar uma tabela de **municípios**, **comarcas**, **circunscrições** e **regiões administrativas** do Tribunal de Justiça de São Paulo, para então visualizar essas informações num mapa. Fazendo curta uma história longa, podemos dizer que:

- uma comarca é o conjunto de um ou mais municípios contíguos (contíguos = municípios que se tocam); 
...
> P.S.: Não se esqueça do ciclo da ciência de dados!

![](/blog/img/data-science.png)

<br/>

As partes de `import` e `tidy` desse projeto processo foi sofrível. Precisei fazer vários web scrapers e diversos códigos para lidar com nomes zuados dos municípios de São Paulo, como Moji/Mogi, Estrela D'Oeste/Doeste, Brodowski/Brodosqui etc. Quem quiser pode dar uma olhada nos códigos do [github da ABJ](https://github.com/abjur/abjMaps).

## O problema

Após passar o dia todo pegando esses dados, finalmente cheguei na parte de `transform` e `visualize`. Nossos objetivos são:
...
Em experiências anteriores já passei por maus bocados tentando completar essa tarefa. Antigamente, a única função conhecida que fazia a união desses conjuntos de formas era a `maptools::unionSpatialPolygons()`, que é contraintuitiva, mal documentada e difícil de usar na prática. Tudo era difícil: desde ler o arquivo `.shp` baixado da internet até a parte de fazer o gráfico. Era R **raiz** mesmo. Mas hoje, para a felicidade de todos, temos a solução **nutella** ;)

> Curiosamente, o Edzer Pebesma, autor do `sf`, também é contributor do `maptools` e mantenedor do `sp`, os pacotes mais importantes de análise espacial pré-tidyverse.
...
O pacote `sf` pode ser um tanto trabalhoso. Se você usa Windows, basta instalar o [Rtools](https://cran.r-project.org/bin/windows/Rtools/) e depois rodar
...
Primeiro, vamos baixar da internet! Obviamente, o melhor lugar para baixar esses arquivos é do FTP do [Instituto Brasileiro de Geografia e Estatística](ftp://geoftp.ibge.gov.br/organizacao_do_territorio/malhas_territoriais/).
...
Agora, queremos juntar essa base com nossa base de comarcas, circunscrições e regiões.
...
Simples assim! Note que agora temos 319 objetos, que é exatamente o número de comarcas. Se quiser adicionar mais variáveis, basta adicionar dentro do `summarise()`:
...
Digamos que você tenha essa lista de mapas
...
E você quer montar mapas com esses títulos

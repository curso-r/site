...
dados, que poodem ser usadas para redução de dimensionalidade ou até mesmo [compressão de arquivos](https://research.googleblog.com/2016/09/image-compression-with-neural-networks.html).
...
Agora vamos estimar $W_1$, $W_2$, $b_1$ e $b_2$ de modo a minimizar algua função de perda.
...
Agora vamos carregar a base de dados e em seguida treinar o nosso àutoencoder`.
...
```{r, echo=FALSE}
encoded_imgs <- readRDS("autoencoder-encoded_imgs.rds")
dim(encoded_imgs)
encoded_imgs[1,]
```
...
```{r echo=FALSE}
plot(readRDS("autoencoder-plot1.rds"))
```
...
```{r, echo = FALSE}
plot(readRDS("autoencoder-plot2.rds"))
```

...
date: "2017-03-27"
tags: ["r", "spotify", "lolapalooza", "tidyverse"]
categories: ["r"]
author: ["Athos"]
banner: "img/banners/lolla-spotify.png"
draft: true
---

Lollapalooza é um famoso festival que acontece em vários lugares do mundo e a edição deste ano em São Paulo contou com 47 bandas distribuídas em quatro palcos. Já o Spotify é um serviço de streaming de músicas que fornece uma API muito amigável para acessarmos dados da base de dados deles.

Daí então achei uma boa ideia brincar com isso para criar um exercício que usa
...
A graça é associar a programação do Lolla com as informações de popularidade das bandas fornecidas pelo Spotify. Abaixo eu vou descrever como peguei os dados, listar as três hipóteses que criei e como gerei os gráficos para estudá-las.
...
**Pacotes** 
```{r, echo=FALSE}
load("2017-03-27-lollapalooza-sp-2017-segundo-spotify/lolla2017.RData")
load("2017-03-27-lollapalooza-sp-2017-segundo-spotify/lolla2017_playlist.RData")
load("2017-03-27-lollapalooza-sp-2017-segundo-spotify/artistas_fora_da_playlist.RData")
load("2017-03-27-lollapalooza-sp-2017-segundo-spotify/lolla2017_programacao.RData")
```
...
O `Rspotify` é um pacote novo e que ainda não está no CRAN, mas já está funcional. Ele é um wrapper para R da API do Spotify.

**Conta no Spotify**

Como se pode imaginar, para utilizar a API do Spotify é necessário ter um cadastro no site deles. 

**App no Spotify**

Esse é o pré-requisito mais burocrático de todos. Para você receber um código de acesso para usar a API deles é preciso criar um *App* dentro da sua conta do Spotify. Eu aprendi a fazer isso seguindo os passos do README do próprio pacote `Rspotify` no Github [(veja aqui)](https://github.com/tiagomendesdantas/Rspotify).
...
Algumas bandas ficaram de fora da playlist e por isso fiz uma pesquisa por nome do artista na própria API do Spotify para recuperar o respectivo id. A função que faz isso é a `searchArtist()` do `Rspotify`.
...
O gráfico acima vai de encontro com o senso comum de que os populares ficam para o final, não ajudando a confirmar a hipótese de que o *Cage The Elefant* estava mal posicionado.
...
### Hipótese III
> **Hipótese III:** A popularidade das bandas nos diferentes palcos estavam equilibradas.
...
O tema tratado aqui não foi útil, mas passamos por quase todas as etapas existentes em um processo típico de análise de dados. Fizemos web scraping, usamos APIs, arrumamos os dados, estruturamos as informações, criamos variáveis e geramos gráficos. Só ficou de fora a parte de modelagem. E não à toa todos os pacotes do tidyverse foram úteis nesse trabalho.

A lição pra casa é encontrar um tema mais útil e aplicar as etapas que aprendemos aqui.
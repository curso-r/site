...
author: ["Julio"]
summary: "A versão 3.4 do R foi lançada nesse final de semana! A atualização tem foco principal na performance. Veja as mudanças."
---
...
A versão 3.4 do R foi lançada nesse final de semana! A atualização tem foco principal na performance. Principais mudanças:
...
```{r, echo=FALSE, fig.cap='R mais rápido! Imagem obtida [aqui](https://github.com/RcppCore).', fig.align='center'}
plot(magick::image_read('img/fastr.jpg'))
```

# Mais sobre o JIT compiler {#jit}

Veja esse exemplo extraído do livro [Efficient R](https://csgillespie.github.io/efficientR/programming.html). Observe a existência de um `<bytecode ...>` na parte de baixo de uma das funções. Isso significa que o pacote `compiler` converteu essa função que pode ser interpretada de forma mais rápida.
...
Para mostrar a mudança, vamos comparar o desempenho das funções usando `microbenchmark()`. Essa função calcula o tempo de execução de uma expressão cem vezes e calcula estatísticas básicas.

...
A gráfico da Figura \@ref(fig:ruim) usa a função `geom_ribbon()` para gerar os intervalos de confiança. Essa função liga os pontos da faixa diretamente, o que é inadequado do ponto de vista estatístico.
...
```{r bom, width=12, height=8, fig.cap="Intervalos de confiança usando `stat = 'stepribbon'́`"}
ggplot(d_km,
       aes(x = time, y = estimate, colour = strata)) +
geom_ribbon(
       aes(ymin = conf.low, ymax = conf.high, fill = strata), 

...
A gráfico da Figura \@ref(fig:ruim) usa a função `geom_ribbon()` para gerar os intervalos de confiança. Essa função liga os pontos da faixa diretamente, o que 
...
Na Figura \@ref(fig:bom), adicionamos o parâmetro `stat = "stepribbon"`, que faz com que o `ggplot` utilize a função `stat_stepribbon()` do `ggalt` para o cálculo da faixa. 

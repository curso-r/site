---
title: "PDF e OCR"
date: "2017-05-27T13:07:31+02:00"
tags: ["r", "pdf", "ocr"]
categories: ["r", "import"]
...
<iframe src="https://docs.google.com/gview?embedded=true&url=https://github.com/curso-r/site/raw/master/content/blog/2017-05-27-ocr/pdf_digital.pdf" seamless="seamless" width="100%" frameBorder="0"  style="height:375px;"></iframe>
...
<iframe src="https://docs.google.com/gview?embedded=true&url=https://github.com/curso-r/site/raw/master/content/blog/2017-05-27-ocr/pdf_digitalizado.pdf" seamless="seamless" width="100%" frameBorder="0"  style="height:375px;"></iframe>
...
Muitas vezes queremos pegar estruturas no texto que dependem da posição dos elementos. Por exemplo, o texto em um PDF pode estar dividido em várias colunas. Para isso, o ideal seria transformar 
...
```{r echo=FALSE}
html <- readRDS('2017-05-27-ocr/html_pdf.rds')
```
...
Apesar de depender do odiado `rJava`, um pacote chato de instalar e configurar, o `tabulizer` é capaz de extrair os dados diretamente para tabelas, de forma simples e intuitiva.
...
<iframe src="https://docs.google.com/gview?embedded=true&url=https://github.com/curso-r/site/raw/master/content/blog/2017-05-27-ocr/pdf_compras.pdf" seamless="seamless" width="100%" frameBorder="0"  style="height:375px;"></iframe>
...
```{r echo=FALSE}
tab <- readRDS('2017-05-27-ocr/tab_raw.rds')
```

Essa função serve para arrumar os nomes zuados que vêm no arquivo
...
O `tesseract` é uma biblioteca escrita em `C` e é uma das mais famosas ferramentas abertas para extração de textos. O pacote em `R` de mesmo nome serve para usar essa biblioteca pelo `R` sem causar dores de cabeça.
...
A principal função do pacote `tesseract` é a `ocr()`. Seu input é uma imagem (pdf, jpeg, tiff, entre outras) e seu output é um texto. Logo, nosso primeiro passo é transformar o pdf em imagem.
...
```{r echo=FALSE}
cat(str_trunc(readRDS("2017-05-27-ocr/txt_ocr.rds"), 300))
```
